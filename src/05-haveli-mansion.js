...
export class HaveliSecurity {
  ...
  removeResident(name, passcode) {
    ...
    let exists = this.#residents.findIndex(r => r.name === name)
    if(exists === -1) return { success: false, message: "Resident nahi mila!" }

    
    this.#residents.splice(exists, 1);
  ...
  isResident(name) {
    // Your code here
    let exists = this.#residents.some(r => r.name === name)
    return exists
  }
}
